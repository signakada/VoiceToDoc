...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    let settings = SettingsStore() // SettingsStoreを先に初期化
    let appState: AppState // AppStateはSettingsStoreに依存するため、initで初期化

    override init() {
        self.appState = AppState(transcriber: WhisperKitTranscriber(), settings: settings)
        super.init()
    }
